...
export const fetchEntry = endpoint => (dispatch, getState) => {
  ...
  .then(
    ...
    .then(payload => dispatch({ type: '@@horseman/ADD_CRAFT_ENTRY', meta: { endpoint }, payload })),
  )
  .catch(() => dispatch({ type: '@@horseman/FETCH_ENTRY_FAIL', meta: { endpoint } }));
};
...
export const fetchCollection = endpoint => (dispatch, getState) => {
  ...
  .then(
    response => response.json()
    .then(payload => dispatch({ type: '@@horseman/ADD_CRAFT_COLLECTION', meta: { endpoint }, payload })),
  )
